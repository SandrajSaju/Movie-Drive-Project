...
import { deleteCastingCall, getCastingCalls, editCastingCall } from '../feature/CastingCalls/castingCallsSlice';
...
const DirectorCastingCalls = () => {
    ...
    const handleConfirmDelete = () => {
        if (selectedCastingCallToDelete) {
            dispatch(deleteCastingCall(selectedCastingCallToDelete._id));
            setIsEditModalOpen(false);
    ...
    // const indexOfLastCastingCall = currentPage * castingCallsPerPage;
    // const indexOfFirstCastingCall = indexOfLastCastingCall - castingCallsPerPage;
    // const currentCastingCalls = castingCalls.slice(indexOfFirstCastingCall, indexOfLastCastingCall);
    ...
    return (
        <>
          { castingCalls && (<section className="text-gray-600 body-font w-4/5 max-md:w-3/5 ml-96 max-md:ml-44">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-col w-full text-center mb-10">
                        <h1 className="text-2xl font-bold title-font mb-4 text-gray-900 tracking-wider">CASTING CALLS</h1>
                        <Link to='/director/addcastingcall' className=' ml-auto '><button className='bg-green-600 w-36 h-10 rounded-xl p-2 text-white font-bold'>Add Casting Call</button></Link>
                    ...
                    {/* <div className='flex justify-center mt-4'>
                        <button
                            ...
                            className={`px-3 py-1 mx-2 focus:outline-none ${currentPage === 1 ? 'bg-gray-300 text-gray-700' : 'bg-green-600 text-white'}`}
                        >
                            Prev
                        </button>
                        {
                            Array.from({ length: pageNumbers }, (_, i) => (
                                <button
                                    ...
                                    className={`px-3 py-1 mx-2 focus:outline-none ${currentPage === i + 1 ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-700'}`}
                                >
                                    {i + 1}
                                </button>
                            ))
                        ...
                        <button
                            ...
                            className={`px-3 py-1 mx-2 focus:outline-none ${currentPage === pageNumbers ? 'bg-gray-300 text-gray-700' : 'bg-green-600 text-white'}`}
                        >
                            Next
                        </button>
                    </div> */}
