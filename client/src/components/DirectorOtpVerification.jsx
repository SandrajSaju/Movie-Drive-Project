...
import { clearDirectorOtpCredentials } from '../feature/Director/directorOtpSlice';
...
const DirectorOtpVerification = () => {
    ...
    const { directorOtpInfo } = useSelector((state) => state.directorOtp)
    console.log(directorOtpInfo);
    const email = directorOtpInfo.email;
    const otp = directorOtpInfo.otp
    const verifyOtp = async () => {
        try {
            await axiosInstance.post('/director/verifyotp', { enteredOtp, email, otp })
